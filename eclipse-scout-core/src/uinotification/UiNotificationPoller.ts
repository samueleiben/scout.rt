 ...
 */
import {ajax, AjaxCall, AjaxError, arrays, BackgroundJobPollingStatus, dates, ErrorHandler, JsonErrorResponse, objects, PropertyChangeEvent, PropertyEventEmitter, PropertyEventMap, scout, Session, UiNotificationDo} from '../index';
...
export class UiNotificationPoller extends PropertyEventEmitter {
  ...
  protected _onSuccess(response: UiNotificationResponse) {
    ...
    if (this.dispatcher && notifications.length) {
