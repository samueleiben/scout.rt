...
describe('uiNotifications', () => {
  ...
  describe('subscribe', () => {
    it('starts the poller if the first topic is subscribed', () => {
      ...
      expect(uiNotifications.pollers.size).toBe(1);

      let poller = uiNotifications.pollers.get('main');
    ...
    it('executes the handlers for the received notifications', async () => {
      ...
      uiNotifications.subscribe('aaa', message => {
        receivedMsgAaa = message;
      });
      let receivedMsgBbb: DoEntity;
      uiNotifications.subscribe('bbb', message => {
        receivedMsgBbb = message;
      });
      let receivedMsgCcc: DoEntity;
      uiNotifications.subscribe('ccc', message => {
        receivedMsgCcc = message;
      });
    ...
    it('starts a poller per system', () => {
      ...
      expect(uiNotifications.pollers.size).toBe(2);

      let poller2 = uiNotifications.pollers.get('sys2');
      expect(poller2.status).toBe('running');
      expect(poller2).not.toBe(poller);

      uiNotifications.subscribe('ccc', () => undefined, 'sys2');
      expect(uiNotifications.pollers.size).toBe(2); // Still 2
    });
  ...
  describe('unsubscribe', () => {
    ...
    it('stopps the poller if the last topic is unsubscribed', () => {
      ...
      expect(uiNotifications.pollers.size).toBe(1); // still 1
  ...
  describe('one', () => {
    it('subscribes for one notification and automatically unsubscribes once it arrives', async () => {
      let receivedMsg: DoEntity;
      uiNotifications.one('aaa', message => {
        receivedMsg = message;
      });
      expect(uiNotifications.pollers.size).toBe(1);
  ...
  describe('when', () => {
    it('same as one but with a promise', done => {
      uiNotifications.when('aaa').then(message => {
    ...
    it('also works with multiple systems', done => {
      uiNotifications.registerSystem('sys2', '/api2/ui-notifications');
      let promises = [];
      let response: UiNotificationResponse = {
        notifications: [{
      ...
      promises.push(uiNotifications.when('aaa').then(message => {
        expect(message).toEqual({
          a: 'aaa'
        } as DoEntity);
        expect(uiNotifications.pollers.get('main')).toBeFalsy();
      }));
      expect(uiNotifications.pollers.get('main')).toBeTruthy();
      ...
      promises.push(uiNotifications.when('aaa', 'sys2').then(message => {
        expect(message).toEqual({
          a: 'aaa'
        } as DoEntity);
        expect(uiNotifications.pollers.get('sys2')).toBeFalsy();
      }));
      expect(uiNotifications.pollers.get('sys2')).toBeTruthy();
